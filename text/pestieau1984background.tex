...
\begin{eqnarray}
\frac{\partial L}{\partial c_{1t}} = \frac{\gamma}{c_{1t}} - \lambda = 0\\
\frac{\partial L}{\partial c_{2t}} = \frac{1-\alpha-\gamma}{c_{2t}}-\lambda\frac{1}{1+r_{t-1}} = 0\\
\frac{\partial L}{\partial b_{2t}} = \frac{\alpha}{a_{t+1} w_t + b_{2t}} - \lambda \frac{n_t}{1+r_{t-1}} = 0\\ 
\frac{\partial L}{\partial \lambda} = w_t a_t + b_t - c_{1t} - \frac{ c_{2t}+ n_t b_{2t}} {1+r_{t-1}}  = 0
\end{eqnarray}


kc: all of the following is not correct. I must incorporate the maximization solution for the above problem into the problem(s) below.

It then follows that optimal first period consumption from the maximization problems yields the following:
...
+ \gamma \ln [ \gamma(b_t+w_t a_{t+1} \frac{1+r_{t-1}+n}{1+r_{t-1}}) ]
+ (1-\alpha-\gamma) \ln c_{2t}\\
+ \lambda [ w a_t + b_t - \gamma(b_t + w_t a_{t+1} \frac{1+r_{t-1}+n_t}{1+r_{t-1}} ) - \frac{c_{2t} + n_t b_{2t}}{1+r_t}]
%\end{equation}
%\end{eqnarray*}
\end{gather*}
...
%kc: left alignment?
\begin{eqnarray}
\frac{\partial L}{\partial b_{2t}} = \frac{\alpha}{w_t a_{t+1} + b_{2t}} - \lambda \frac{n_t}{1+r_t} = 0\\
\frac{\partial L}{\partial c_{2t}} = \frac{1-\alpha-\gamma}{c_{2t}}-\lambda\frac{1}{1+r_t} = 0\\
\frac{\partial L}{\partial \lambda} = w_t a_t + b_t - \gamma(b_t+w_t a_{t+1} \frac{1+r_{t-1}+n_t}{1+r_{t-1}} ) - \frac{c_{2t} + n_t b_{2t}}{1+r_t} = 0
\end{eqnarray}
...
\frac{\partial L}{\partial c_{2t}} = \frac{1-\alpha-\gamma}{c_{2t}}-\lambda\frac{1}{1+r_t} = 0\\
\frac{\partial L}{\partial \lambda} = w a_t + b_t - \gamma(b+w a_t \frac{1+r_{t-1}+n}{1+r_{t-1}} ) - \frac{c_{2} + n b_{2t}}{1+r_t} = 0
\end{eqnarray}
