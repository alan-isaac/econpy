...
class testDstat(unittest.TestCase):
	...
	def test_descriptive(self):
		...
		self.assertAlmostEqual(d.m2 , np.var(x))
		self.assertAlmostEqual(d.std , np.std(x, ddof=1))
		#assertEqual(d.zscores , Sstats.zs(x))
		self.assertAlmostEqual(d.median , np.median(x))
		'''
...
class testBiometry(unittest.TestCase):
	def test_welchs_approximate_ttest(self):
		...
		female = (10, 8.5, np.sqrt(3.6)/np.sqrt(10))
		male = (10, 4.8, np.sqrt(0.9)/np.sqrt(10))
