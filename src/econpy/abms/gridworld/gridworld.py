"""Provides classes for implementing agent-based simulations.
...
Note that a ``GridWorldGUI`` is an ``Observer``:
it does not subclass ``GridWorld``
but rather owns an instance of ``GridWorld``.

About observers: if you want to collect info from
or display your world, use an observer, such as GridWorldGUI.
Use a world observer to create agent observers
and patch observers using the class attributes
_PatchObserverType and _AgentObserverType.
If you subclass a world observer,
consider overriding these attributes.

:note: Draw the following semantic distinction:
  a world has locations, which can be occupied by
  objects, which have *positions*.
  Coordinates that do not represent a world location
  may be replaced by None but are generally transformed
...
# from matplotlib.backends.backend_tkagg import NavigationToolbar2TkAgg #removed??
...
#More fundamentally, they are dicts mapping agents
#to a constrained set of locations and positions.
...
class BoundedLocationMap(LocationMap):
    ...
    Invalid coordinates are wrapped! (NEW; and this may change!)
    ...
    def __str__(self):
        return "{} by {} topology.".format(*self._shape)
    #changed 20190220, WARNING illegal coordinates are now adjusted (not None) 
    def location(self, coordinates):
        """Return tuple, the location.
        (Overriding parent class to constrain location.)
        :note: On a 2 x 2 grid, possible coordinates range from -0.5 to 1.5.
        """
        shape = self.shape
        if self._ndim != len(coordinates):
            msg = 'Shape {0} does not match coordinates {1}'
            raise ValueError(msg.format(shape,coordinates))
        result = tuple(((0.5 + cn) % sn) - 0.5 for (cn,sn) in zip(coordinates, shape))
        assert all((-0.5 < cn < sn - 0.5) for (cn,sn) in zip(result, shape))
        return result
    #added 20190220, WARNING may be changed or removed
    def random_locations(self, nlocs, exclude=False, prng=None):
        """Return list of `nlocs` random locations from the grid.
        Locations have integer coordinates.  (May change!)
        `prng` must have a `randrange` method.
        Multiple occupance is allowed.  (May change!)
        So `exclude` is ingored.
        """
        logging.debug('Enter BoundedLocationMap.random_locations.')
        if prng is None:
            logging.info("No prng passed; using random.")
            prng = random
        if nlocs != abs(int(nlocs)):
            errmsg = '{0} is not a positive integer.'.format(nlocs)
            raise ValueError(errmsg)
        shape = self._shape
        n_possible = reduce(operator.mul, shape)
        locations = list(tuple(map(prng.randrange, shape)) for _ in range(nlocs))
        assert len(locations) == nlocs
        logging.debug('Exit BoundedLocationMap.random_locations.')
        return locations
    #properties
    # read-only
    @property
...
class FiniteGrid(BoundedLocationMap):
    ...
    def location(self, coordinates):
        ...
        User-provided `coordinates` are rounded to integers!
        """
        shape = self.shape
        ...
        location = tuple( xi%si for (xi,si) in zip(coordinates,shape) )
...
class TorusGrid(FiniteGrid):
    ...
    def location(self, coordinates):
        ...
        return tuple( xi%si for (xi,si) in zip(coordinates,shape) )


################## END TOPOLOGIES


################## BEGIN OBSERVABLE TYPES
...
class Observable(object):
    ...
    @property
    def observers(self):
        return tuple(self._observers)  #20190224 coerce to tuple
...
class WorldBase(Observable):
    ...
    def __str__(self):
        nPatches = 0 if (self.patches is None) else len(tuple(self.patches))
        nAgents = 0 if (self.agents is None) else len(tuple(self.agents))
        return """World:
        {}
        patches: {}
        agents: {}
        """.format(self._topology, nPatches, nAgents)
    def initialize(self):
        """Return None.
    ...
    def setup(self):
        ...
        (Often called by a SetUp button in a GUI.)
        """
    ...
    def run(self, maxiter=None):
        ...
        (Do the model setup before calling `run`.)
        """
    ...
    def create_agents(self, AgentType, number=0, locations=None, prng=None):
        ...
        Specify either the number of agents or the locations for agents.
    ...
    def patch_at(self, coordinates, preconstrained=False):
        ...
        """
        :note: assumes _patches is a tuple!
        :note: thanks to Steven D'Aprano
          for suggesting operator.getitem to replace tuple.__getitem__
...
class Agent(Observable):
    ...
    """
    #convenience declarations, anticipating possible display
    _fillcolor = 'black'
    _shape = 'classic'
    _shapesize = (0.3,0.3)
    def __init__(self, world=None, position=(0,0), **kwargs):
        self._world = world
        self._initial_position = position  #chkchk unused
        self._position = position
        #follow tutle.py and use an orientation vector:
        self._orient = (1,0)  #initial orientation (in degrees)
    ...
    def forward(self, distance):
        ...
        #rotate (d,0) to heading and add
        xy = vadd(self._position, smul(self._orient, distance))
    ...
    def set_heading(self, angle):
        """Return None; change self._orient to match `angle`,
        given in clockwise degrees from the horizontal.
        """
        self._orient = rotate2d((1,0), angle) #assumes stand form
        self.notify_observers('setheading', to_angle=angle)
    #####   patch related methods
    ...
    def display(self, **kwargs):
        ...
        for key, val in kwargs.items():
            if key in dspkeys:
                ...
                invalid.append(key)
        if invalid:
            msg = """Unknown display attributes: {0}.
            Valid display attributes are {1}."""
            raise ValueError(msg.format(invalid,dspkeys))
...
class NavigatorAgent(Agent, turtle.TNavigator):
    ...
    def __init__(self, world=None, position=(0,0)):
        turtle.TNavigator.__init__(self)  #sets pos to (0,0)!
...
def smul(v, s):
    """Return tuple, the vector `v` scaled by scalar `s`.
    """
    return tuple(s * vi for vi in v)

def vadd(v1,v2):
    """Return tuple, the vector sum `v1+v2`.
    """
    assert len(v1) == len(v2), "vectors not conformable"
    return tuple(v1i + v2i for (v1i,v2i) in zip(v1,v2))
    

def rotate2d(v, angle):
    """Return tuple, the 2d vector `v` rotated counterclockwise by `angle`,
    where `angle` is in degrees.  Recall that the counterclockwise
    rotation matrix is [[c,-s],[s,c]].
    """
    angle = math.radians(angle % 360) #convert to radians
...
class GridWorldGUI(Observer, tk.Frame):
    ...
    def setup_display(self):
        logging.info("Enter GridWorldGUI.setup_display.")
        self.setup_turtlescreen() #setup screen
        self._setup_button_frame()
        self._setup_slider_frame()
        self._setup_monitor_frame()
        self._setup_graph_frame()
        self._setup()
        logging.info("Exit GridWorldGUI.setup_display.")
    ...
    def add_button(self, label, callback):
        ...
        """
        if isinstance(callback, str): #must be an attribute of the subject
    ...
    def add_plot(self, title, datafunc, **kwargs):
        """Return None.
        Creates a time-series plot request, which will display in the GUI.
        The `datafunc` should return one number for plotting each time it is called.
        Depends on Matplotlib.  See Matplotlib documentation for kwargs.
        :see: http://matplotlib.sourceforge.net/api/pyplot_api.html#matplotlib.pyplot.hist
        """
    ...
    def _setup_graph_frame(self):
        ...
        for ct, graph in enumerate(graphs):
            ...
            if kind == 'histogram':
                ...
                logging.warn('Ignoring unknown graph type: {0}'.format(kind))
    ...
    def _setup_graphs(self):
        logging.info("Enter _setup_graphs.")
        for graph in self._graphs:
            graph.setup()
        logging.info("Exit _setup_graphs.")
    ...
    def _tracer(self, val):
        ...
        try:
            ...
            (This is normal when the observer exits.)"""
            logging.info(msg)
    #PROPERTIES
    # read-only
    @property
...
class TSPlot(FigureCanvasTkAgg):
    ...
    def set_background(self):
        ...
        of the canvas.
        """
        logging.info("end TSPlot.set_background")
        ax = self._ax
        ax.clear()
        ydata = self._ydata
